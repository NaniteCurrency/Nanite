...
#include "ui_znttcontroldialog.h"
...
std::list<std::string> ZNttControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNttControlDialog::listMints;

ZNttControlDialog::ZNttControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNttControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNttControlDialog::~ZNttControlDialog()
{
    delete ui;
}

void ZNttControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNttControlDialog::updateList()
{
...
void ZNttControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNttControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNtt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNttControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNttControlDialog::GetSelectedMints()
{
...
void ZNttControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
