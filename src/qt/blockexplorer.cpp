...
extern double GetDifficulty(const CBlockIndex* blockindex = NULL, int algo=0);
...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::NTT, nValue);
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Difficulty"), strprintf("%.4f", GetDifficulty(pBlock, block.GetAlgo())),
            _("Bits"), utostr(block.nBits),
            _("Nonce"), utostr(block.nNonce),
            _("Version"), itostr(block.nVersion),
            _("Hash"), "<pre>" + block.GetPoWHash(block.GetAlgo()).GetHex() + "</pre>",
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (nanite.conf).");
            QMessageBox::warning(this, "NANITE Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
