...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NTT);
    ...
    if (!settings.contains("nAnonymizeNaniteAmount"))
        settings.setValue("nAnonymizeNaniteAmount", 1000);

    nAnonymizeNaniteAmount = settings.value("nAnonymizeNaniteAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNaniteAmount"))
        SoftSetArg("-anonymizenaniteamount", settings.value("nAnonymizeNaniteAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nanite.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNaniteAmount:
            return QVariant(nAnonymizeNaniteAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNaniteAmount:
            nAnonymizeNaniteAmount = value.toInt();
            settings.setValue("nAnonymizeNaniteAmount", nAnonymizeNaniteAmount);
            emit anonymizeNaniteAmountChanged(nAnonymizeNaniteAmount);
            break;
        case CoinControlFeatures:
