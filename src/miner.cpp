   ...
   // else
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zNtt spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zNtt serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Track how many getdata requests this block gets
    {
        ...
        wallet.mapRequestCount[pblock->GetPoWHash(pblock->GetAlgo())] = 0;
    }

    // Process this block the same as if we had received it from another node
    CValidationState state;
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NANITEMiner : ProcessNewBlock, block not accepted");

    for (CNode* node : vNodes) {
        node->PushInventory(CInv(MSG_BLOCK, pblock->GetPoWHash(pblock->GetAlgo())));
    }
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake, miningAlgo));
        if (!pblocktemplate.get())
            continue;
        ...
        //Stake miner main
        if (fProofOfStake) {
            LogPrintf("CPUMiner : proof-of-stake block found %s \n", pblock->GetPoWHash(pblock->GetAlgo()).ToString().c_str());
        ...
        LogPrintf("Running NANITEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
