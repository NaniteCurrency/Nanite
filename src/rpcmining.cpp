...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or nanite.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    if (fGenerate && Params().MineBlocksOnDemand()) {
        ...
        while (nHeight < nHeightEnd) {
            unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwalletMain, false, miningAlgo));
            ...
            while (!CheckProofOfWork(pblock->GetPoWHash(pblock->GetAlgo()), pblock->nBits, pblock->GetAlgo())) {
            ...
            ++nHeight;
            blockHashes.push_back(pblock->GetPoWHash(pblock->GetAlgo()).GetHex());
        }
        return blockHashes;
...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_untt / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in untt) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in untt); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in untt)\n"
...
// Release the wallet and main lock while waiting
    ...
    if (pindexPrev != chainActive.Tip() ||
        ...
        pblocktemplate = CreateNewBlock(scriptDummy, pwalletMain, false, miningAlgo);
        if (!pblocktemplate)
...
protected:
    ...
    {
        if (block.GetPoWHash(block.GetAlgo()) != hash)
            return;
        found = true;
        state = stateIn;
    };
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    ...
    CValidationState state;
    submitblock_StateCatcher sc(block.GetPoWHash(block.GetAlgo()));
