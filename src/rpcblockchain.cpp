...
double GetDifficulty(const CBlockIndex* blockindex, int algo)
{
    ...
    if (blockindex == NULL) {
        if (chainActive.Tip() == NULL)
            ...
            blockindex = GetLastBlockIndex4Algo(chainActive.Tip(), algo);
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hash", block.GetPoWHash(block.GetAlgo()).GetHex()));
    int confirmations = -1;
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        znttObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    znttObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zNTTsupply", znttObj));
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in nanite\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of nanite addresses\n"
            "     \"naniteaddress\"   	 	(string) nanite address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
