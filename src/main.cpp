...
#if defined(NDEBUG)
#error "NANITE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in untt) are considered zero fee (for relaying and mining)
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNtt spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zNtt spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zNtt spend with serial %s from tx %s is not in valid range\n",
    ...
    CBlockIndex* pindexSlow = NULL;
    {
        ...
        if (fTxIndex) {
            ...
            if (pblocktree->ReadTxIndex(hash, postx)) {
                ...
                hashBlock = header.GetPoWHash(header.GetAlgo());
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex)
{
    if (!ReadBlockFromDisk(block, pindex->GetBlockPos()))
        return false;
    if (block.GetPoWHash(block.GetAlgo()) != pindex->GetBlockHash()) {
        LogPrintf("%s : block=%s index=%s\n", __func__, block.GetPoWHash(block.GetAlgo()).ToString().c_str(), pindex->GetBlockHash().ToString().c_str());
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < (Params().NetworkID() == CBaseChainParams::TESTNET ? 11800 : 11811) && nHeight >= 11522) {
	nSubsidy = 65972222 * COIN;
    } else if (nHeight < Params().LAST_POW_BLOCK() && nHeight >= 11811) {
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 42799 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 17000 * COIN;
    } else if (nHeight <= 71599 && nHeight >= 42800) {
        nSubsidy = 16000 * COIN;
    } else if (nHeight <= 100399 && nHeight >= 71600) {
        nSubsidy = 15000 * COIN;
    } else if (nHeight <= 129199 && nHeight >= 100400) {
        nSubsidy = 14000 * COIN;
    } else if (nHeight <= 157999 && nHeight >= 129200) {
        nSubsidy = 13000 * COIN;
    } else if (nHeight <= 186799 && nHeight >= 158000) {
        nSubsidy = 12000 * COIN;
    } else if (nHeight <= 215599 && nHeight >= 186800) {
        nSubsidy = 11000 * COIN;
    } else if (nHeight <= 244399 && nHeight >= 215600) {
        nSubsidy = 10000 * COIN;
    } else if (nHeight <= 273199 && nHeight >= 244400) {
        nSubsidy = 9000 * COIN;
    } else if (nHeight <= 618799 && nHeight >= 273200) {
        nSubsidy = 4500 * COIN;
    } else if (nHeight <= 1273599 && nHeight >= 618800) {
        nSubsidy = 4000 * COIN;
    } else if (nHeight <= 2475199 && nHeight >= 1273600) {
        nSubsidy = 3500 * COIN;
    } else if (nHeight <= 3022399 && nHeight >= 2475200) {
        nSubsidy = 3000 * COIN;
    } else if (nHeight >= 3022400) {
        nSubsidy = 2500 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NANITE
...
{
    RenameThread("nanite-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZNTTMinted()
{
...
void RecalculateZNTTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNTT supply
...
bool RecalculateNTTSupply(int nHeightStart)
{
...
{
    // NANITE: recalculate Accumulator Checkpoints that failed to database properly
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (block.GetPoWHash(block.GetAlgo()) == Params().HashGenesisBlock()) {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zNtt with serial %s is already in the block %d\n",
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZNTTMinted();
        RecalculateZNTTSpent();
        RecalculateNTTSupply(Params().Zerocoin_StartHeight());
    }
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNttSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert NTT to zNTT
...
bool ActivateBestChain(CValidationState& state, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetPoWHash(pblock->GetAlgo()) == pindexMostWork->GetBlockHash() ? pblock : NULL, fAlreadyChecked))
                return false;
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    uint256 hash = block.GetPoWHash(algo);
    BlockMap::iterator it = mapBlockIndex.find(hash);
    if (it != mapBlockIndex.end())
        return it->second;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(GetAlgo(block.nVersion)), block.nBits, GetAlgo(block.nVersion)))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Check timestamp
    LogPrint("debug", "%s: block=%s  is proof of stake=%d\n", __func__, block.GetPoWHash(block.GetAlgo()).ToString().c_str(), block.IsProofOfStake());
        ...
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
                ...
                BOOST_FOREACH (const CTxIn& in, tx.vin) {
                    if (mapLockedInputs.count(in.prevout)) {
                        if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                            mapRejectedBlocks.insert(make_pair(block.GetPoWHash(block.GetAlgo()), GetTime()));
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNtt spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNtt serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    int algo = block.GetAlgo();
    if (pindexPrev == NULL)
        return error("%s : null pindexPrev for block %s", __func__, block.GetPoWHash(algo).ToString().c_str());

    unsigned int nBitsRequired = GetNextWorkRequired(pindexPrev, &block, algo);
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            return state.DoS(100, error("%s : prev block height=%d hash=%s is invalid, unable to add block %s", __func__, pindexPrev->nHeight, block.hashPrevBlock.GetHex(), block.GetPoWHash(block.GetAlgo()).GetHex()),
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.GetPoWHash(block.GetAlgo()) != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            return state.DoS(100, error("%s : prev block %s is invalid, unable to add block %s", __func__, block.hashPrevBlock.GetHex(), block.GetPoWHash(block.GetAlgo()).GetHex()),
                             REJECT_INVALID, "bad-prevblk");
        }
    }

    if (block.GetPoWHash(block.GetAlgo()) != Params().HashGenesisBlock() && !CheckWork(block, pindexPrev))
        return false;
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNtt mints and %d zNtt spends\n", __func__, nMints, nSpends);
    ...
    if (pblock->GetPoWHash(pblock->GetAlgo()) != Params().HashGenesisBlock() && pfrom != NULL) {
...

    {
        ...
        MarkBlockAsReceived (pblock->GetPoWHash(pblock->GetAlgo()));
        if (!checked) {
            return error ("%s : CheckBlock FAILED for block %s", __func__, pblock->GetPoWHash(pblock->GetAlgo()).GetHex());
        }
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false)) {
        ...
        if (vSortedByHeight.size() > nHeightLastBlockFile && pcoinsTip->GetBestBlock() != vSortedByHeight[nHeightLastBlockFile].second->GetBlockHash()) {
            ...
            while (nSortedPos < vSortedByHeight.size()) {
                ...
                uint256 hashBlock = block.GetPoWHash(block.GetAlgo());
                for (unsigned int i = 0; i < block.vtx.size(); i++) {
                    CValidationState state;
                    CTxUndo undoDummy;
                    if (i > 0)
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(block, it->second)) {
                            LogPrintf("%s: Processing out of order child %s of %s\n", __func__, block.GetPoWHash(block.GetAlgo()).ToString(),
                                head.ToString());
                            CValidationState dummy;
                            if (ProcessNewBlock(dummy, NULL, &block, &it->second)) {
                                nLoaded++;
                                queue.push_back(block.GetPoWHash(block.GetAlgo()));
                            }
                        }
                        range.first++;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NANITE: We use certain sporks during IBD, so check to see if they are
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
                ...
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0)
                        Misbehaving(pfrom->GetId(), nDoS);
                    std::string strError = "invalid header received " + header.GetPoWHash(header.GetAlgo()).ToString();
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            CValidationState state;
            if (!mapBlockIndex.count(block.GetPoWHash(block.GetAlgo()))) {
