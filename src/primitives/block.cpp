...
uint256 CBlockHeader::GetPoWHash(int algo) const
{
        switch (algo)
        {
            ...
            case ALGO_BLAKE:
                return HashBlake(BEGIN(nVersion), END(nAccumulatorCheckpoint));
            case ALGO_X17:
    ...
    if(!IsProofOfStake())
    {
        for(unsigned int i = 0; i < vtx[0].vout.size(); i++)
        {
            ...
            if (whichType == TX_PUBKEY)
            {
                ...
                if (!key.Sign(GetPoWHash(GetAlgo()), vchBlockSig))
                     return false;
...
bool CBlock::CheckBlockSignature() const
{
    ...
    if (whichType == TX_PUBKEY)
    {
        ...
        return pubkey.Verify(GetPoWHash(GetAlgo()), vchBlockSig);
    }
